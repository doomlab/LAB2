...
columns = ["number_features", "precision0", "recall0", "f1 score0", 
          "precision1", "recall1", "f1score1", "accuracy"]
index = np.arange(100, 4001, 100).tolist()
log_results = pd.DataFrame(index=index, columns=columns)
log_results = log_results.fillna(0)

##create the loop here 
#for i in np.arange(100, 4001, 100).tolist()
i = 100
...
#predict
y_training = lr.predict(cv_training_text)
y_dev = lr.predict(cv_dev_text)
y_testing = lr.predict(cv_testing_text)

#classification
report = classification_report(training_answer, y_training, output_dict=True)
[list(report["0"].values()), list(report["1"].values()), report["accuracy"]]
