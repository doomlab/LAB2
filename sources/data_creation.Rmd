...
no_correct <- subset(combo_data, 
                     !grepl("^correction|^erratum", combo_data$TITLE))

##exclude duplicates that are simply small text changes
stringdist()

write.csv(as.data.frame(combo_data), "output_data/combined_data_unedited.csv", row.names = F)
```

## Hand Fixing the Output

There are often values that need to be handchecked for repetition. First, we will import the previously checked file.

```{r}
library(readxl)
previously_checked <- read_excel("output_data/previously_checked.xlsx")
previously_excluded <- subset(previously_checked,
                              duplicate == "exclude")
